...
export function preferRecommendedVoice(
  ...
  const region = target !== family ? target : normalizeForLookup(table.defaultRegion);
  const deviceOs = Array.isArray(platform?.os) ? platform.os : [];
  const deviceBrowser = Array.isArray(platform?.browser) ? platform.browser : [];

  const pairs = [];
  table.voices.forEach((entry, index) => {
    if (!platformAccepts(entry.os, deviceOs) || !platformAccepts(entry.browser, deviceBrowser)) {
      return;
    }
